...
/**
 * An OpenGL surface on an AWT Canvas, allowing OpenGL to be embedded in a Swing application. All OpenGL calls are done
 * on the EDT. This is slightly less efficient then a dedicated thread, but greatly simplifies synchronization. Note
 * that you may need to call {@link #stop()} or a Swing application may deadlock on System.exit due to how LWJGL and/or
 * Swing deal with shutdown hooks.
 * 
 * @author Nathan Sweet
 */
public class LwjglAWTCanvas implements Application {
	...
	public LwjglAWTCanvas(ApplicationListener listener, boolean useGL2, LwjglAWTCanvas sharedContextCanvas) {
		...
		try {
			canvas = new AWTGLCanvas(GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice(),
					new PixelFormat(), sharedDrawable) {
	...
	/**
	 * Make the canvas' context current. It is highly recommended that the context is only made current inside the AWT
	 * thread (for example in an overridden paintGL()).
	 */
