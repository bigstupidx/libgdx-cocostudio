...
public class LwjglApplicationConfiguration {
	...
	/**
	 * Adds a window icon. Icons are tried in the order added, the first one that works is used. Typically three icons
	 * should be provided: 128x128 (for Mac), 32x32 (for Windows and Linux), and 16x16 (for Windows).
	 */
	...
	public static DisplayMode[] getDisplayModes() {
		...
		for (java.awt.DisplayMode mode : displayModes) {
			...
			if (mode.getBitDepth() != desktopMode.getBitDepth())
				continue;
			modes.add(new LwjglApplicationConfigurationDisplayMode(mode.getWidth(), mode.getHeight(), mode
					.getRefreshRate(), mode.getBitDepth()));
		}
