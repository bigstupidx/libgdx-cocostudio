...
public final class Intersector {
	/**
	 * Returns the lowest positive root of the quadric equation given by a* x * x + b * x + c = 0. If no solution is
	 * given Float.Nan is returned.
	 * 
	 * @param a
	...
	/**
	 * Returns whether the given point is inside the triangle. This assumes that the point is on the plane of the
	 * triangle. No check is performed that this is the case.
	 * 
	 * @param point
	 *            the point
	 * @param t1
	...
	/**
	 * Determines on which side of the given line the point is. Returns -1 if the point is on the left side of the line,
	 * 0 if the point is on the line and 1 if the point is on the right side of the line. Left and right are relative to
	...
	/**
	 * Checks whether the line segment and the circle intersect and returns by how much and in what direction the line
	 * has to move away from the circle to not intersect.
	 * 
	 * @param start
	...
	/**
	 * Intersects a {@link Ray} and a {@link Plane}. The intersection point is stored in intersection in case an
	...
	/**
	 * Intersects a line and a plane. The intersection is returned as the distance from the first point to the plane. In
	 * case an intersection happened, the return value is in the range [0,1]. The intersection point can be recovered by
	 * point1 + t * (point2 - point1) where t is the return value of this method.
	 * 
	 * @param x
	 * @param y
	 * @param z
	...
	public static float intersectLinePlane(float x, float y, float z, float x2, float y2, float z2, Plane plane,
		...
		if (denom != 0) {
			...
			if ((t >= 0 && t <= 1) && intersection != null)
				intersection.set(origin).add(direction.mul(t));
			return t;
	...
	/**
	 * Check whether specified convex polygons overlap. If they do, optionally obtain a Minimum Translation Vector
	 * indicating the minimum magnitude vector required to push the polygons out of the collision.
	 * 
	 * @param p1
	 *            The first polygon.
	 * @param p2
	 *            The second polygon.
	 * @param mtv
	 *            A Minimum Translation Vector to fill in the case of a collision (optional).
	 * @return Whether polygons overlap.
	 */
	...
	/**
	 * Check whether polygons defined by the given vertex arrays overlap. If they do, optionally obtain a Minimum
	 * Translation Vector indicating the minimum magnitude vector required to push the polygons out of the collision.
	 * 
	 * @param verts1
	...
	/**
	 * Splits the triangle by the plane. The result is stored in the SplitTriangle instance. Depending on where the
	 * triangle is relative to the plane, the result can be:
	 * 
	 * <ul>
	 * <li>Triangle is fully in front/behind: {@link SplitTriangle#front} or {@link SplitTriangle#back} will contain the
	 * original triangle, {@link SplitTriangle#total} will be one.</li>
	 ...
	 * </ul>
	 * 
	 * The input triangle should have the form: x, y, z, x2, y2, z2, x3, y3, y3. One can add additional attributes per
	 * vertex which will be interpolated if split, such as texture coordinates or normals. Note that these additional
	 * attributes won't be normalized, as might be necessary in case of normals.
	 * 
	 * @param triangle
	 * @param plane
	 * @param split
