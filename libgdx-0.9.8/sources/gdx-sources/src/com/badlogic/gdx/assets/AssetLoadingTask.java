 ...
 * {@link Callable} and is used with an {@link ExecutorService threadpool} to load parts of an asset asynchronously if
 * the asset is loaded with an {@link AsynchronousAssetLoader}.
 * 
 * @author mzechner
 */
class AssetLoadingTask implements Callable<Void> {
	 ...
	 * loaders {@link AsynchronousAssetLoader#loadAsync(AssetManager, String, AssetLoaderParameters)} method is first
	 * called on a worker thread. Once this method returns, the rest of the asset is loaded on the rendering thread via
			...
			if (depsFuture == null) {
				...
				if (depsFuture.isDone()) {
					try {
						...
						throw new GdxRuntimeException("Couldn't load dependencies of asset '" + assetDesc.fileName
								+ "'", e);
					}
					dependenciesLoaded = true;
					if (asyncDone) {
