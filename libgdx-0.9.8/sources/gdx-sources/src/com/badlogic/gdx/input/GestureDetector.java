...
/**
 * {@link InputProcessor} implementation that detects gestures (tap, long press, fling, pan, zoom, pinch) and hands them
 * to a {@link GestureListener}.
 * 
 * @author mzechner
 */
public class GestureDetector extends InputAdapter {
	 ...
	 * longPressDuration=1.1f, maxFlingDelay=0.15f.
	 */
	 ...
	 * flings, panning or pinch zooming. Each method returns a boolean indicating if the event should be handed to the
	...
	public static interface GestureListener {
		...
		/**
		 * Called when a tap occured. A tap happens if a touch went down on the screen and was lifted again without
		 * moving outside of the tap square. The tap square is a rectangular area around the initial touch position as
		 * specified on construction time of the {@link GestureDetector}.
		 * 
		 * @param count
		 *            the number of taps.
		 */
		...
		/**
		 * Called when the user dragged a finger over the screen and lifted it. Reports the last known velocity of the
		 * finger in pixels per second.
		 * 
		 * @param velocityX
		...
		/**
		 * Called when the user performs a pinch zoom gesture. The original distance is the distance in pixels when the
		 * gesture started.
		 * 
		...
		/**
		 * Called when a user performs a pinch zoom gesture. Reports the initial positions of the two involved fingers
