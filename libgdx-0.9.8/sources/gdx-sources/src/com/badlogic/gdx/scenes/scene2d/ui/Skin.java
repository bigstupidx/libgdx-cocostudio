...
public class Skin implements Disposable {
	...
	/**
	 * Adds all named txeture regions from the atlas. The atlas will not be automatically disposed when the skin is
	 * disposed.
	 */
	 ...
	 * created from the region and stored in the skin. If the region is an {@link AtlasRegion} then the
	 * {@link AtlasRegion#splits} are used, otherwise the ninepatch will have the region as the center patch.
	 */
	 ...
	 * "getStyle" method or the style was not found in the skin, no exception is thrown and the actor is left unchanged.
	 */
			...
			private void readNamedObjects(Json json, Class type, ObjectMap<String, ObjectMap> valueMap) {
				...
				for (Entry<String, ObjectMap> valueEntry : valueMap.entries()) {
					...
					try {
						...
						throw new SerializationException("Error reading " + type.getSimpleName() + ": "
								+ valueEntry.key, ex);
					}
				}
			}
