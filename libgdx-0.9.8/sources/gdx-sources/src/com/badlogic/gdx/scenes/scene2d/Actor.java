...
/**
 * 2D scene graph node. An actor has a position, rectangular size, origin, scale, rotation, and color. The position
 * corresponds to the unrotated, unscaled bottom left corner of the actor. The position is relative to the actor's
 * parent. The origin is relative to the position and is used for scale and rotation.
 * <p>
 * An actor also has a list of actions that can manipulate the actor over time, and a list of listeners that are
 * notified of events the actor receives.
 * 
 * @author mzechner
 * @author Nathan Sweet
 */
public class Actor {
	 ...
	 * This draw method} is convenient to draw a rotated and scaled TextureRegion. {@link SpriteBatch#begin()} has
	 * already been called on the SpriteBatch. If {@link SpriteBatch#end()} is called to draw without the SpriteBatch
	 * then {@link SpriteBatch#begin()} must be called before the method returns.
	 * <p>
	...
	/**
	 * Sets this actor as the event {@link Event#setTarget(Actor) target} and propagates the event to this actor and
	 * ancestor actors as necessary. If this actor is not in the stage, the stage must be set before calling this
	 * method.
	 * <p>
	 * Events are fired in 2 phases. The first phase notifies listeners on each actor starting at the root and
	 * propagating downward to (and including) this actor. The second phase notifes listeners on each actor starting at
	 * this actor and, if {@link Event#getBubbles()} is true, propagating upward to the root. If the event is
	 * {@link Event#stop() stopped} at any time, it will not propagate to the next actor.
	 * 
	 ...
	 * listeners, this actor is set as the {@link Event#getListenerActor() listener actor}. The event
	 * {@link Event#setTarget(Actor) target} must be set before calling this method. If this actor is not in the stage,
	 ...
	 * {@link #isVisible() visible}, or null if no actor was hit. The point is specified in the actor's local coordinate
	 * system (0,0 is the bottom left of the actor and width,height is the upper right).
	 * <p>
	 ...
	 * 
	 * @param touchable
	 *            If true, the hit detection will respect the {@link #setTouchable(Touchable) touchability}.
	 * @see Touchable
	 */
	...
	/**
	 * Sets the z-index of this actor. The z-index is the index into the parent's {@link Group#getChildren() children},
	 * where a lower index is below a higher index. Setting a z-index higher than the number of children will move the
	...
	/**
	 * Clips the specified screen aligned rectangle, specified relative to the transform matrix of the stage's
	 * SpriteBatch. The transform matrix and the stage's camera must not have rotational components. Calling this method
	 * must be followed by a call to {@link #clipEnd()} if true is returned.
	 * 
	...
	public boolean clipBegin(float x, float y, float width, float height) {
		...
		ScissorStack.calculateScissors(stage.getCamera(), stage.getSpriteBatch().getTransformMatrix(), tableBounds,
				scissorBounds);
	...
	/**
	 * Transforms the specified point in the actor's coordinates to be in the stage's coordinates. Note this method will
	 * ONLY work for screen aligned, unrotated, unscaled actors!
	 */
	...
	/**
	 * Transforms the specified point in the actor's coordinates to be in the parent's coordinates. Note this method
	 * will ONLY work for screen aligned, unrotated, unscaled actors!
	 */
	...
	/**
	 * Converts coordinates for this actor to those of a parent actor. The ascendant does not need to be a direct
	 * parent.
	 */
