 ...
 * performant and is prefered over vertex arrays if hardware supports it.
 * </p>
 * 
 * <p>
 ...
 * and the alias of the attribute must match exactly for this to work. For OpenGL ES 1.x rendering this aliases are
 * irrelevant.
 * </p>
 * 
 * <p>
...
public class Mesh implements Disposable {
	 ...
	 * @param attributes
	 *            the {@link VertexAttribute}s. Each vertex attribute defines one property of a vertex such as position,
	 ...
	 *            the maximum number of indices this mesh can hold
	 * @param attributes
	 *            the {@link VertexAttributes}. Each vertex attribute defines one property of a vertex such as position,
	 *            normal or texture coordinate
	 */
...
// if (type == VertexDataType.VertexArray && Gdx.graphics.isGL20Available()) type = VertexDataType.VertexBufferObject;
	...
	/**
	 * Create a new Mesh that is a combination of transformations of the supplied base mesh. Not all primitive types,
	 * like line strip and triangle strip, can be combined.
	 * 
	 * @param isStatic
	 *            whether this mesh is static or not. Allows for internal optimizations.
	 * @param transformations
	 *            the transformations to apply to the meshes
	...
	/**
	 * Create a new Mesh that is a combination of the supplied meshes. The meshes must have the same VertexAttributes
	 * signature. Not all primitive types, like line strip and triangle strip, can be combined.
	 * 
	 * @param isStatic
	 *            whether this mesh is static or not. Allows for internal optimizations.
	 * @param meshes
	...
	/**
	 * Create a new Mesh that is a combination of the supplied meshes. The meshes must have the same VertexAttributes
	 * signature. If transformations is supplied, it must have the same length as meshes. Not all primitive types, like
	 * line strip and triangle strip, can be combined.
	 * 
	 * @param isStatic
	 *            whether this mesh is static or not. Allows for internal optimizations.
	 * @param meshes
	...
	public static Mesh create(boolean isStatic, final Mesh[] meshes, final Matrix4[] transformations) {
		...
		for (int i = 1; i < meshes.length; i++) {
			...
			if (transformations != null)
				transform(transformations[i], vertices, vertexSize, offset, numComponents, voffset / vertexSize, vsize
						/ vertexSize);
	...
	/**
	 * Sets the vertices of this Mesh. The attributes are assumed to be given in float format. If this mesh is
	 * configured to use fixed point an IllegalArgumentException will be thrown.
	 * 
	 * @param vertices
	 *            the vertices.
	 */
	 ...
	 * one of the {@link #render(int)} methods or not. Usually you want to use autobind. Manual binding is an expert
	 * functionality. There is a driver bug on the MSM720xa chips that will fuck up memory if you manipulate the
	 * vertices and indices of a Mesh multiple times while it is bound. Keep this in mind.
	 * 
	 * @param autoBind
	...
	/**
	 * Binds the underlying {@link VertexArray}/{@link VertexBufferObject} and {@link IndexBufferObject} if indices were
	...
	/**
	 * Unbinds the underlying {@link VertexArray}/{@link VertexBufferObject} and {@link IndexBufferObject} is indices
	 * were given. Use this with OpenGL ES 1.x and when auto-bind is disabled.
	 */
	public void unbind() {
		if (Gdx.graphics.isGL20Available())
			throw new IllegalStateException("can't use this render method with OpenGL ES 2.0");
	 ...
	 * 
	 * <p>
	 * This method is intended for use with OpenGL ES 1.x and will throw an IllegalStateException when OpenGL ES 2.0 is
	 * used.
	 * </p>
	 * 
	 * @param primitiveType
	...
	/**
	 * <p>
	 * Renders the mesh using the given primitive type. offset specifies the offset into vertex buffer and is ignored
	 * for the index buffer. Count specifies the number of vertices or indices to use thus count / #vertices per
	 * primitive primitives are rendered.
	 * </p>
	 * 
	 * <p>
	 * This method is intended for use with OpenGL ES 1.x and will throw an IllegalStateException when OpenGL ES 2.0 is
	 ...
	 *            the primitive type
	 * @param offset
	 *            the offset into the vertex buffer, ignored for indexed rendering
	 * @param count
