...
public class MipMapGenerator {
	...
	/**
	 * Sets the image data of the {@link Texture} based on the {@link Pixmap}. The texture must be bound for this to
	 * work. If <code>disposePixmap</code> is true, the pixmap will be disposed at the end of the method.
	 * 
	 * @param pixmap
	 *            the Pixmap
	 * @param disposePixmap
	...
	private static void generateMipMapGLES20(Pixmap pixmap, boolean disposePixmap) {
		Gdx.gl.glTexImage2D(GL20.GL_TEXTURE_2D, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(),
		...
		if (Gdx.graphics.isGL20Available()
			...
			if ((Gdx.gl20 == null) && textureWidth != textureHeight)
				throw new GdxRuntimeException(
						"texture width and height must be square when using mipmapping in OpenGL ES 1.x");
	...
	private static void generateMipMapCPU(Pixmap pixmap, int textureWidth, int textureHeight, boolean disposePixmap) {
		Gdx.gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(),
				0, pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
		if ((Gdx.gl20 == null) && textureWidth != textureHeight)
		...
		while (width > 0 && height > 0) {
			...
			Gdx.gl.glTexImage2D(GL10.GL_TEXTURE_2D, level, pixmap.getGLInternalFormat(), pixmap.getWidth(),
					pixmap.getHeight(), 0, pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
