...
public class Decal {
	...
	/**
	 * The transformation offset can be used to change the pivot point for rotation and scaling. By default the pivot is
	 * the middle of the decal.
	 */
	...
	protected void transformVertices() {
		/**
		 * It would be possible to also load the x,y,z into a Vector3 and apply all the transformations using already
		 * existing methods. Especially the quaternion rotation already exists in the Quaternion class, it then would
		 * look like this:
		 * ----------------------------------------------------------------------------------------------------
		 ...
		 * ---------------------------------------------------------------------------------------------------- However,
		 * a half ass benchmark with dozens of thousands decals showed that doing it "by hand", as done here, is about
		 * 10% faster. So while duplicate code should be avoided for maintenance reasons etc. the performance gain is
