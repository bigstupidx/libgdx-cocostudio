...
/**
 * Contains an atlas of tiles by tile id for use with {@link TileMapRenderer} It does not need to be loaded with packed
 * files. Just plain textures.
 * 
 * @author Tomas Lazaro
 */
public class SimpleTileAtlas extends TileAtlas {
	...
	private static TextureRegion[][] split(Texture texture, int totalWidth, int totalHeight, int width, int height,
		...
		for (int x = 0; x < xSlices; x++) {
			for (int y = 0; y < ySlices; y++) {
				res[x][y] = new TextureRegion(texture, margin + x * (width + spacing), margin + y * (height + spacing),
						width, height);
			}
		}
		return res;
