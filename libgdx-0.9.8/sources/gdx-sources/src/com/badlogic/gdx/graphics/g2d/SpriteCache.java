...
/**
 * Draws 2D images, optimized for geometry that does not change. Sprites and/or textures are cached and given an ID,
 * which can later be used for drawing. The size, color, and texture region for each cached image cannot be modified.
 * This information is stored in video memory and does not have to be sent to the GPU each time it is drawn.<br>
 * <br>
 * To cache {@link Sprite sprites} or {@link Texture textures}, first call {@link SpriteCache#beginCache()}, then call
 * the appropriate add method to define the images. To complete the cache, call {@link SpriteCache#endCache()} and store
 * the returned cache ID.<br>
 * <br>
 * To draw with SpriteCache, first call {@link #begin()}, then call {@link #draw(int)} with a cache ID. When SpriteCache
 * drawing is complete, call {@link #end()}.<br>
 * <br>
 * By default, SpriteCache draws using screen coordinates and uses an x-axis pointing to the right, an y-axis pointing
 * upwards and the origin is the bottom left corner of the screen. The default transformation and projection matrices
 * can be changed. If the screen is {@link ApplicationListener#resize(int, int) resized}, the SpriteCache's matrices
 * must be updated. For example:<br>
 ...
 * (<i>Gdx.gl.glEnable(GL10.GL_BLEND);</i>) and set the blend func as needed before or between calls to
 * {@link #draw(int)}.<br>
 * <br>
 * SpriteCache is managed. If the OpenGL context is lost and the restored, all OpenGL resources a SpriteCache uses
 * internally are restored.<br>
 * <br>
...
public class SpriteCache implements Disposable {
	 ...
	 * 
	 * @param size
	 *            The maximum number of images this cache can hold. The memory required to hold the images is allocated
	 *            up front.
	 * @param useIndices
	...
	public SpriteCache(int size, ShaderProgram shader, boolean useIndices) {
		...
		mesh = new Mesh(true, size * (useIndices ? 4 : 6), useIndices ? size * 6 : 0, new VertexAttribute(
	...
	/**
	 * Starts the redefinition of an existing cache, allowing the add and {@link #endCache()} methods to be called. If
	 * this is not the last cache created, it cannot have more entries added to it than when it was first created. To do
	 * that, use {@link #clear()} and then {@link #begin()}.
	 */
	...
	/**
	 * Adds the specified vertices to the cache. Each vertex should have 5 elements, one for each of the attributes: x,
	 * y, color, u, and v. If indexed geometry is used, each image should be specified as 4 vertices, otherwise each
	 * image should be specified as 6 vertices.
	 */
	 ...
	 * texture coordinates attribute is called called "a_texCoords", the color attribute is called "a_color". The
	 * projection matrix is uploaded via a mat4 uniform called "u_proj", the transform matrix is uploaded via a uniform
	 * called "u_trans", the combined transform and projection matrx is is uploaded via a mat4 uniform called
