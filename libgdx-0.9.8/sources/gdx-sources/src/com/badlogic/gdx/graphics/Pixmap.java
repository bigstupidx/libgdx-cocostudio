...
/**
 * <p>
 * A Pixmap represents an image in memory. It has a width and height expressed in pixels as well as a {@link Format}
 * specifying the number and order of color components per pixel. Coordinates of pixels are specified with respect to
 * the top left corner of the image, with the x-axis pointing to the right and the y-axis pointing downwards.
 * </p>
 * 
 * <p>
 * By default all methods use blending. You can disable blending with {@link Pixmap#setBlending(Blending)}. The
 * {@link Pixmap#drawPixmap(Pixmap, int, int, int, int, int, int, int, int)} method will scale and stretch the source
 * image to a target image. There either nearest neighbour or bilinear filtering can be used.
 * </p>
 * 
 * <p>
 * A Pixmap stores its data in native heap memory. It is mandatory to call {@link Pixmap#dispose()} when the pixmap is
 * no longer needed, otherwise memory leaks will result
 * </p>
 * 
...
public class Pixmap implements Disposable {
	...
	/**
	 * Creates a new Pixmap instance from the given file. The file must be a Png, Jpeg or Bitmap. Paletted formats are
	 * not supported.
	 * 
	 * @param file
	 *            the {@link FileHandle}
...
// public void setStrokeWidth (int width);
	...
	/**
	 * Draws an area form another Pixmap to this Pixmap. This will automatically scale and stretch the source image to
	 * the specified target rectangle. Use {@link Pixmap#setFilter(Filter)} to specify the type of filtering to be used
	...
	/**
	 * Fills a rectangle starting at x, y extending by width to the right and by height downwards (y-axis points
	 * downwards) using the current color.
	 * 
	 * @param x
	 *            The x coordinate
	...
	/**
	 * Returns the direct ByteBuffer holding the pixel data. For the format Alpha each value is encoded as a byte. For
	 * the format LuminanceAlpha the luminance is the first byte and the alpha is the second byte of the pixel. For the
	 * formats RGB888 and RGBA8888 the color components are stored in a single byte each in the order red, green, blue
	 * (alpha). For the formats RGB565 and RGBA4444 the pixel colors are stored in shorts in machine dependent order.
	 * 
