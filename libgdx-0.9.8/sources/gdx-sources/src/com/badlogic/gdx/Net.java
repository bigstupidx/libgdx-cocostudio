...
/**
 * Provides methods to perform networking operations, such as simple HTTP get and post requests, and TCP server/client
 * socket communication.</p>
 * 
 * To perform an HTTP request create a {@link HttpRequest} with the HTTP method (see {@link HttpMethods} for common
 * methods) and invoke {@link #sendHttpRequest(HttpRequest, HttpResponseListener)} with it and a
 * {@link HttpResponseListener}. After the HTTP request was processed, the {@link HttpResponseListener} is called with a
 * {@link HttpResponse} with the HTTP response values and an status code to determine if the request was successful or
 * not.
 * 
 * To create a TCP client socket to communicate with a remote TCP server, invoke the
 * {@link #newClientSocket(Protocol, String, int, SocketHints)} method. The returned {@link Socket} offers an
...
public interface Net {
	...
	/**
	 * Contains information about the HTTP status line returned with the {@link HttpResponse} after a
	 * {@link HttpRequest} was performed.
	 */
	public static class HttpStatus {
		...
		/**
		 * Returns the status code of the HTTP response, normally 2xx status codes indicate success while 4xx and 5xx
		 * indicate client and server errors, respectively (see <a
	...
	/**
	 * HTTP response interface with methods to get the response data as a byte[], a {@link String} or an
	 * {@link InputStream}.
	 */
	public static interface HttpResponse {
		 ...
		 * 
		 * @return the result as a byte[] or null in case of a timeout or if the operation was canceled/terminated
		 *         abnormally. The timeout is specified when creating the HTTP request, with
		 ...
		 * 
		 * @return the result as a string or null in case of a timeout or if the operation was canceled/terminated
		 *         abnormally. The timeout is specified when creating the HTTP request, with
		 *         {@link HttpRequest#setTimeOut(int)}
		 */
	 ...
	 * <ul>
	 * <li><strong>httpMethod:</strong> GET or POST are most common, can use {@link Net.HttpMethods HttpMethods} for
	 * static references</li>
	...
	public static class HttpRequest {
		 ...
		 * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html">HTTP/1.1: Header Field Definitions</a>
		 * document.
		 * 
		 * @param name
		 ...
		 * 
		 * @param content
		 *            A string encoded in the corresponding Content-Encoding set in the headers, with the data to send
		 *            with the HTTP request. For example, in case of HTTP GET, the content is used as the query string
		 *            of the GET while on a HTTP POST it is used to send the POST data.
		 */
		...
		/**
		 * Sets the time to wait for the HTTP request to be processed, use 0 block until it is done. The timeout is used
		 * for both the timeout when establishing TCP connection, and the timeout until the first byte of data is
		 * received.
		 * 
		 * @param timeOut
		 *            the number of milliseconds to wait before giving up, 0 or negative to block until the operation is
		 *            done
		 */
	...
	public static interface HttpResponseListener {
		 ...
		 * {@link HttpResponse} contains the {@link HttpStatus} and should be used to determine if the request was
		 * successful or not (see more info at {@link HttpStatus#getStatusCode()}). For example:
		 * 
		 * <pre>
		...
		/**
		 * Called if the {@link HttpRequest} failed because an exception when processing the HTTP request, could be a
		 * timeout any other reason (not an HTTP error).
		 * 
		 * @param t
	 ...
	 *            The {@link HttpResponseListener} to call once the HTTP response is ready to be processed. Could be
	...
	/**
	 * Creates a new server socket on the given port, using the given {@link Protocol}, waiting for incoming
	 * connections.
	 * 
	 * @param port
	 *            the port to listen on
	 * @param hints
	 *            additional {@link ServerSocketHints} used to create the socket. Input null to use the default setting
	 ...
	 *            the port
	 * @param hints
	 *            additional {@link SocketHints} used to create the socket. Input null to use the default setting
	 *            provided by the system.
	...
	/**
	 * Launches the default browser to display a URI. If the default browser is not able to handle the specified URI,
	 * the application registered for handling URIs of the specified type is invoked. The application is determined from
	 * the protocol and path of the URI.
	 * 
	 * @param URI
