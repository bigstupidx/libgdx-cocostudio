...
/**
 * An implementation of the {@link Application} interface for Android. Create an {@link Activity} that derives from this
 * class. In the {@link Activity#onCreate(Bundle)} method call the {@link #initialize(ApplicationListener, boolean)}
...
public class AndroidApplication extends Activity implements Application {
	...
	private static void readUniqueId() {
		String user_id = Secure.getString(((Activity) Gdx.app).getContentResolver(), Secure.ANDROID_ID);
		if (user_id == null || user_id.equals("9774d56d682e549c")) {
			ismadeinchina = true;
			SharedPreferences preferences = ((Activity) Gdx.app).getSharedPreferences("Setting", 0);
	...
	private static boolean ismadeinchina = false;
...
//		return unique_id;
	...
	/**
	 * This method has to be called in the {@link Activity#onCreate(Bundle)} method. It sets up all the things necessary
	 * to get input, render via OpenGL and so on. If useGL20IfAvailable is set the AndroidApplication will try to create
	 * an OpenGL ES 2.0 context which can then be used via {@link Graphics#getGL20()}. The {@link GL10} and {@link GL11}
	 * interfaces should not be used when OpenGL ES 2.0 is enabled. To query whether enabling OpenGL ES 2.0 was
	 * successful use the {@link Graphics#isGL20Available()} method. Uses a default
	...
	/**
	 * This method has to be called in the {@link Activity#onCreate(Bundle)} method. It sets up all the things necessary
	 * to get input, render via OpenGL and so on. If config.useGL20 is set the AndroidApplication will try to create an
	 * OpenGL ES 2.0 context which can then be used via {@link Graphics#getGL20()}. The {@link GL10} and {@link GL11}
	 * interfaces should not be used when OpenGL ES 2.0 is enabled. To query whether enabling OpenGL ES 2.0 was
	 * successful use the {@link Graphics#isGL20Available()} method. You can configure other aspects of the application
	 * with the rest of the fields in the {@link AndroidApplicationConfiguration} instance.
	 * 
	 * @param listener
	 *            the {@link ApplicationListener} implementing the program logic
	 * @param config
	 *            the {@link AndroidApplicationConfiguration}, defining various settings of the application (use
	 *            accelerometer, etc.).
	 */
