...
/**
 * The Class AffineTransform represents a linear transformation (rotation, scaling, or shear) followed by a translation
 * that acts on a coordinate space. It preserves collinearity of points and ratios of distances between collinear
 * points: so if A, B, and C are on a line, then after the space has been transformed via the affine transform, the
 * images of the three points will still be on a line, and the ratio of the distance from A to B with the distance from
 * B to C will be the same as the corresponding ratio in the image space.
 * 
 * @since Android 1.0
 */
public class CGAffineTransform {
	...
	/**
	 * Instantiates a new affine transform by specifying the values of the 2x3 transformation matrix as doubles. The
	 * type is set to the default type: <code>TYPE_UNKNOWN</code>
	 * 
	 * @param m00
	 *            the m00 entry in the transformation matrix.
	 * @param m10
	...
	/**
	 * Instantiates a new affine transform by reading the values of the transformation matrix from an array of floats.
	 * The mapping from the array to the matrix starts with <code>matrix[0]</code> giving the top-left entry of the
	 * matrix and proceeds with the usual left-to-right and top-down ordering.
	 * <p/>
	...
	/**
	 * Writes the values of the transformation matrix into the given array of doubles. If the array has length 4, only
	 * the linear transformation part will be written into it. If it has length greater than 4, the translation vector
	 ...
	 * type to <code>TYPE_IDENTITY</code> if the resulting AffineTransformation is the identity transformation,
	...
	/**
	 * Creates a new AffineTransformation that is a translation alone with the translation vector given by the values
	 * sent as parameters. The new transformation's type is <code>TYPE_IDENTITY</code> if the AffineTransformation is
	 * the identity transformation, otherwise it's <code>TYPE_TRANSLATION</code>.
	 * 
	 * @param mx
	 *            the distance to translate in the x direction.
	 * @param my
	...
	/**
	 * Changes the current AffineTransform the one obtained by taking the transform t and applying this AffineTransform
	 * to it.
	 * 
	 * @param t
	 ...
	 *             if this AffineTransform cannot be inverted (the determinant of the linear transformation part is
	 *             zero).
	 */
...
//            throw new NoninvertibleTransformException("Determinant is zero");
	 ...
	 *            the original point.
	 * @param dst
	 *            Point2D object to be filled with the destination coordinates (where the original point is sent by this
	 *            AffineTransform). May be null.
	 * @return the point in the AffineTransform's image space where the original point is sent.
	 */
	 ...
	 * @param srcOff
	 *            the offset in the source point array of the first point to be transformed.
	 * @param dst
	 *            the point array where the images of the points (after applying the AffineTransformation) should be
	 *            placed.
	 * @param dstOff
	 *            the offset in the destination array where the new values should be written.
	 * @param length
	 *            the number of points to transform.
	 * @throws ArrayIndexOutOfBoundsException
	 *             if <code>srcOff + length > src.length</code> or <code>dstOff + length > dst.length</code>.
	 */
	...
	/**
	 * Applies this AffineTransform to a set of points given as an array of double values where every two values in the
	 * array give the coordinates of a point; the even-indexed values giving the x coordinates and the odd-indexed
	 * values giving the y coordinates.
	 * 
	 * @param src
	 *            the array of points to be transformed.
	 * @param srcOff
	 ...
	 *            the number of points to transform.
	 * @throws ArrayIndexOutOfBoundsException
	 *             if <code>srcOff + length*2 > src.length</code> or <code>dstOff + length*2 > dst.length</code>.
	 */
	...
	/**
	 * Applies this AffineTransform to a set of points given as an array of float values where every two values in the
	 * array give the coordinates of a point; the even-indexed values giving the x coordinates and the odd-indexed
	 * values giving the y coordinates.
	 * 
	 * @param src
	 ...
	 * Applies this AffineTransform to a set of points given as an array of float values where every two values in the
	 * array give the coordinates of a point; the even-indexed values giving the x coordinates and the odd-indexed
	 * values giving the y coordinates. The destination coordinates are given as values of type <code>double</code>.
	 * 
	 * @param src
	 *            the array of points to be transformed.
	 * @param srcOff
	 ...
	 * Applies this AffineTransform to a set of points given as an array of double values where every two values in the
	 * array give the coordinates of a point; the even-indexed values giving the x coordinates and the odd-indexed
	 * values giving the y coordinates. The destination coordinates are given as values of type <code>float</code>.
	 * 
	 * @param src
	 *            the array of points to be transformed.
	 * @param srcOff
	...
	/**
	 * Transforms the point according to the linear transformation part of this AffineTransformation (without applying
	 * the translation).
	 * 
	 * @param src
	 *            the original point.
	 * @param dst
	 *            the point object where the result of the delta transform is written.
	...
	/**
	 * Applies the linear transformation part of this AffineTransform (ignoring the translation part) to a set of points
	 * given as an array of double values where every two values in the array give the coordinates of a point; the
	 ...
	 *            the offset in the source point array of the first point to be transformed.
	 * @param dst
	 *            the point array where the images of the points (after applying the delta transformation) should be
	 *            placed.
	 * @param dstOff
	 *            the offset in the destination array where the new values should be written.
	 * @param length
	 ...
	 *            the original point.
	 * @param dst
	 *            the point object where the result of the inverse transform is written (may be null).
	 * @return the result of applying the inverse transform. Inverse transform.
	 * @throws NoninvertibleTransformException
	 *             if this AffineTransform cannot be inverted (the determinant of the linear transformation part is
	 *             zero).
	 */
	public CGPoint inverseTransform(CGPoint src, CGPoint dst) throws NoninvertibleTransformException {
		double det = getDeterminant();
		if (Math.abs(det) < ZERO) {
	...
	/**
	 * Applies the inverse of this AffineTransform to a set of points given as an array of double values where every two
	 * values in the array give the coordinates of a point; the even-indexed values giving the x coordinates and the
	 ...
	 *            the offset in the source point array of the first point to be transformed.
	 * @param dst
	 *            the point array where the images of the points (after applying the inverse of the
